...
public class DesignateServerAspect {
    ...
    public Object execute(ProceedingJoinPoint point, DesignateServer designateServer) throws Throwable {
        ...
        if (appId == null) {
            throw new CocoJobException("can't find appId in params for:" + signature.toString());
        }

        // 获取执行机器
        AppInfoDO appInfo = appInfoRepository.findById(appId).orElseThrow(() -> new CocoJobException("can't find app info"));
        ...
        if (!askResponse.isSuccess()) {
            throw new CocoJobException("remote process failed: " + askResponse.getMessage());
        }
