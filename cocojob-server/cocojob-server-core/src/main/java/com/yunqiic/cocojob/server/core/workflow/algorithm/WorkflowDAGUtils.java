package com.yunqiic.cocojob.server.core.workflow.algorithm;
...
public class WorkflowDAGUtils {
    ...
    /**
     * Add by zhangchunsheng on 2021/02/08
    ...
    public static WorkflowDAG convert(PEWorkflowDAG peWorkflowDAG) {
        ...
        if (peWorkflowDAG.getNodes() == null || peWorkflowDAG.getNodes().isEmpty()) {
            throw new CocoJobException("empty graph");
        }
        ...
        peWorkflowDAG.getEdges().forEach(edge -> {
            ...
            if (from == null || to == null) {
                throw new CocoJobException("Illegal Edge: " + JsonUtils.toJSONString(edge));
            }
        ...
        // 合法性校验（至少存在一个顶点）
        if (rootIds.isEmpty()) {
            throw new CocoJobException("Illegal DAG: " + JsonUtils.toJSONString(peWorkflowDAG));
        }
