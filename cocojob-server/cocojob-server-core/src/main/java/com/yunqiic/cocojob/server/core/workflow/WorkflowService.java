...
public class WorkflowService {
    ...
    private String validateAndConvert2String(Long wfId, PEWorkflowDAG dag) {
        ...
        if (!WorkflowDAGUtils.valid(dag)) {
            throw new CocoJobException("illegal DAG");
        }
        ...
        for (PEWorkflowDAG.Node node : dag.getNodes()) {
            WorkflowNodeInfoDO nodeInfo = workflowNodeInfoRepository.findById(node.getNodeId()).orElseThrow(() -> new CocoJobException("can't find node info by id :" + node.getNodeId()));
            // 更新工作流 ID
            ...
            if (!wfId.equals(nodeInfo.getWorkflowId())) {
                throw new CocoJobException("can't use another workflow's node");
            }
            // 只保存节点的 ID 信息，清空其他信息
    ...
    public List<WorkflowNodeInfoDO> saveWorkflowNode(List<SaveWorkflowNodeRequest> workflowNodeRequestList) {
        ...
        for (SaveWorkflowNodeRequest req : workflowNodeRequestList) {
            ...
            if (!appId.equals(req.getAppId())) {
                throw new CocoJobException("node list must are in the same app");
            }
            ...
            if (req.getType() == WorkflowNodeType.JOB) {
                ...
                if (!jobInfoDO.getAppId().equals(appId)) {
                    throw new CocoJobException("Permission Denied! can't use other app's job!");
                }
    ...
    private WorkflowInfoDO permissionCheck(Long wfId, Long appId) {
        ...
        if (!wfInfo.getAppId().equals(appId)) {
            throw new CocoJobException("Permission Denied! can't operate other app's workflow!");
        }
        return wfInfo;
    }
}
