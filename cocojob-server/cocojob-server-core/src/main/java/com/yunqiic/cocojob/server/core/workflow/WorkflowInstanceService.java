...
import com.yunqiic.cocojob.server.persistence.remote.model.WorkflowInstanceInfoDO;
...
public class WorkflowInstanceService {
    ...
    public void stopWorkflowInstance(Long wfInstanceId, Long appId) {
        ...
        if (!WorkflowInstanceStatus.GENERALIZED_RUNNING_STATUS.contains(wfInstance.getStatus())) {
            throw new CocoJobException("workflow instance already stopped");
        }
        // 停止所有已启动且未完成的服务
    ...
    /**
     * Add by zhangchunsheng on 2021/02/07
     *
     * @param wfInstanceId 工作流实例ID
     * @param appId        应用ID
     */
    ...
    public void retryWorkflowInstance(Long wfInstanceId, Long appId) {
        ...
        // 仅允许重试 失败的工作流
        if (WorkflowInstanceStatus.GENERALIZED_RUNNING_STATUS.contains(wfInstance.getStatus())) {
            throw new CocoJobException("workflow instance is running");
        }
        if (wfInstance.getStatus() == WorkflowInstanceStatus.SUCCEED.getV()) {
            throw new CocoJobException("workflow instance is already successful");
        }
        // 因为 DAG 非法 或者 因任务信息缺失 而失败的工作流实例无法重试
        if (SystemInstanceResult.CAN_NOT_FIND_JOB.equals(wfInstance.getResult())) {
            throw new CocoJobException("you can't retry the workflow instance which is missing job info!");
        }
        // 校验 DAG 信息
        PEWorkflowDAG dag;
        try {
            ...
            throw new CocoJobException("you can't retry the workflow instance whose DAG is illegal!");
        }
        // 检查当前工作流信息
        ...
        if (!workflowInfo.isPresent() || workflowInfo.get().getStatus() == SwitchableStatus.DISABLE.getV()) {
            throw new CocoJobException("you can't retry the workflow instance whose metadata is unavailable!");
        }
    ...
    public WorkflowInstanceInfoDO fetchWfInstance(Long wfInstanceId, Long appId) {
        ...
        if (!Objects.equals(appId, wfInstance.getAppId())) {
            throw new CocoJobException("Permission Denied!");
        }
        return wfInstance;
    }

    /**
     * Add by zhangchunsheng on 2021/02/20
     * 将节点标记成功
    ...
    public void markNodeAsSuccess(Long appId, Long wfInstanceId, Long nodeId) {
        ...
        // 校验工作流实例状态，运行中的不允许处理
        if (WorkflowInstanceStatus.GENERALIZED_RUNNING_STATUS.contains(wfInstance.getStatus())) {
            throw new CocoJobException("you can't mark the node in a running workflow!");
        }
        // 这里一定能反序列化成功
        PEWorkflowDAG dag = JSON.parseObject(wfInstance.getDag(), PEWorkflowDAG.class);
        ...
        if (targetNode == null) {
            throw new CocoJobException("can't find the node in current DAG!");
        }
        ...
        // 其他情况均拒绝处理
        throw new CocoJobException("you can only mark the node which is failed and not allow to skip!");
