package com.yunqiic.cocojob.server.core.workflow;
...
import com.yunqiic.cocojob.common.SystemInstanceResult;
import com.yunqiic.cocojob.common.WorkflowContextConstant;
...
import com.yunqiic.cocojob.server.core.DispatchService;
import com.yunqiic.cocojob.server.core.instance.InstanceService;
import com.yunqiic.cocojob.server.core.lock.UseSegmentLock;
import com.yunqiic.cocojob.server.core.service.UserService;
import com.yunqiic.cocojob.server.core.uid.IdGenerateService;
import com.yunqiic.cocojob.server.core.workflow.algorithm.WorkflowDAGUtils;
import com.yunqiic.cocojob.server.extension.defaultimpl.alram.AlarmCenter;
import com.yunqiic.cocojob.server.extension.defaultimpl.alram.module.WorkflowInstanceAlarm;
import com.yunqiic.cocojob.server.persistence.remote.model.*;
import com.yunqiic.cocojob.server.persistence.remote.repository.JobInfoRepository;
import com.yunqiic.cocojob.server.persistence.remote.repository.WorkflowInfoRepository;
import com.yunqiic.cocojob.server.persistence.remote.repository.WorkflowInstanceInfoRepository;
import com.yunqiic.cocojob.server.persistence.remote.repository.WorkflowNodeInfoRepository;

import javax.annotation.Resource;
import java.util.*;

import static com.yunqiic.cocojob.server.core.workflow.algorithm.WorkflowDAGUtils.isNotAllowSkipWhenFailed;
...
public class WorkflowInstanceManager {
     ...
     * 创建工作流任务实例
     * ********************************************
     * 2021-02-03 modify by zhangchunsheng
    ...
    public Long create(WorkflowInfoDO wfInfo, String initParams, Long expectTriggerTime) {
        ...
        try {
            ...
            if (!WorkflowDAGUtils.valid(dag)) {
                ...
                throw new CocoJobException(SystemInstanceResult.INVALID_DAG);
            }
            // 初始化节点信息
            initNodeInfo(dag);
            ...
            if (dbNum < allJobIds.size()) {
                ...
                throw new CocoJobException(SystemInstanceResult.CAN_NOT_FIND_JOB);
            }
    ...
    private void initNodeInfo(PEWorkflowDAG dag) {
        for (PEWorkflowDAG.Node node : dag.getNodes()) {
            WorkflowNodeInfoDO workflowNodeInfo = workflowNodeInfoRepository.findById(node.getNodeId()).orElseThrow(()->new CocoJobException(SystemInstanceResult.CAN_NOT_FIND_NODE));
            ...
            if (workflowNodeInfo.getType() == null || workflowNodeInfo.getType() == WorkflowNodeType.JOB.getCode()) {
                ...
                if (workflowNodeInfo.getJobId() == null) {
                    throw new CocoJobException(SystemInstanceResult.ILLEGAL_NODE);
                }
                JobInfoDO jobInfo = jobInfoRepository.findById(workflowNodeInfo.getJobId()).orElseThrow(()->new CocoJobException(SystemInstanceResult.CAN_NOT_FIND_JOB));
