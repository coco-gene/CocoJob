...
public class ServerDiscoveryService {
    ...
    private final CocoJobWorkerConfig config;
    ...
    public ServerDiscoveryService(Long appId, CocoJobWorkerConfig config) {
        this.appId = appId;
        this.config = config;
    }
    ...
    public void start(ScheduledExecutorService timingPool) {
        ...
        if (org.springframework.util.StringUtils.isEmpty(this.currentServerAddress) && !config.isEnableTestMode()) {
            throw new CocoJobException("can't find any available server, this worker has been quarantined.");
        }
