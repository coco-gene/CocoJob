package com.yunqiic.cocojob.samples.config;
...
import com.yunqiic.cocojob.official.processors.impl.sql.SpringDatasourceSqlProcessor;
...
public class SqlProcessorConfiguration {
    ...
    @Bean
    @DependsOn({"initCocoJob"})
    public DataSource sqlProcessorDataSource() {
        String path = System.getProperty("user.home") + "/test/h2/" + CommonUtils.genUUID() + "/";
        String jdbcUrl = String.format("jdbc:h2:file:%scocojob_sql_processor_db;DB_CLOSE_DELAY=-1;DATABASE_TO_UPPER=false", path);
